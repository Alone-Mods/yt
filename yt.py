import subprocess import os import shutil from pathlib import Path
...
downloads_dir = os.path.expanduser('~/storage/downloads/Yt_downloads') os.makedirs(downloads_dir, exist_ok=True)

def sanitize_filename(name): name = name.strip().replace(" ", "_") return name

def download_video(): url = input(f"{colors['cyan']}Enter YouTube video URL: {colors['reset']}").strip() if not url: print(f"{colors['red']}Error: URL is required.{colors['reset']}") return
...
def download_audio(): url = input(f"{colors['cyan']}Enter YouTube audio URL: {colors['reset']}").strip() if not url: print(f"{colors['red']}Error: URL is required.{colors['reset']}") return
...
def main(): print(f"{styles['bold']}{colors['magenta']}\nYOUTUBE MEDIA TOOL - ASH{colors['reset']}") print(f"{colors['blue']}1. Download Video") print("2. Download Audio") print("0. Exit" + colors['reset'])
